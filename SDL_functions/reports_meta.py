...
# function to get statistical threshold using FDR correction
def get_statistical_threshold(zmap_path: str, 
                            pmap_path: str, 
                            mask1_path: str, 
                            fdr_alpha: float = 0.05) -> Tuple[float, np.ndarray, np.ndarray]:
    """
    Calculate the threshold of statistical significance for NIfTI images using FDR correction.
    
    Parameters:
    -----------
    zmap_path : str
        Path to the z-map NIfTI file
    pmap_path : str
        Path to the p-map NIfTI file
    mask1_path : str
        Path to the inclusive mask NIfTI file
    fdr_alpha : float, default=0.05
        FDR correction alpha level
        
    Returns:
    --------
    tuple : (threshold, mask2, fdr_corrected_pvals)
        is_significant : bool
            Whether any voxels survived FDR correction
        threshold : float
            The minimum absolute z-value within the FDR-corrected significant voxels
        mask2 : np.ndarray
            Binary mask of voxels surviving FDR correction (p < fdr_alpha) within mask1
        fdr_corrected_pvals : np.ndarray
            FDR-corrected p-values within mask1
            
    Raises:
    -------
    ValueError
        If no voxels survive FDR correction or if input files have mismatched dimensions
    FileNotFoundError
    ...
    try:
        ...
        raise FileNotFoundError(f"Could not load input file: {e}")
    
    # Check that all images have the same shape
    if not (zmap_data.shape == pmap_data.shape == mask1_data.shape):
        raise ValueError(f"Input images have mismatched dimensions: "
                        f"zmap: {zmap_data.shape}, pmap: {pmap_data.shape}, "
                        f"mask1: {mask1_data.shape}")
    
    # Extract p-values within the inclusive mask
    pvals_in_mask = pmap_data[mask1_data]
    
    # Check for valid p-values
    valid_pvals = (pvals_in_mask >= 0) & (pvals_in_mask <= 1) & np.isfinite(pvals_in_mask)
    
    if not np.any(valid_pvals):
        raise ValueError("No valid p-values found within the mask")
    
    # Filter to only valid p-values
    pvals_valid = pvals_in_mask[valid_pvals]
    
    # Perform FDR correction using scipy's false_discovery_control
    # This implements the Benjamini-Hochberg procedure
    fdr_corrected = false_discovery_control(pvals_valid, method='bh')
    
    # Create array to hold FDR-corrected p-values for all voxels in mask
    fdr_corrected_full = np.ones_like(pvals_in_mask)  # Initialize with 1.0
    fdr_corrected_full[valid_pvals] = fdr_corrected
    
    # Create mask2: voxels with FDR-corrected p < alpha within mask1
    mask2_1d = fdr_corrected_full < fdr_alpha
    
    # Convert back to 3D mask
    mask2 = np.zeros_like(mask1_data, dtype=bool)
    mask2[mask1_data] = mask2_1d
    
    # Check if any voxels survive FDR correction
    if not np.any(mask2):
        warnings.warn(f"No voxels survive FDR correction at alpha = {fdr_alpha}")
        return np.max(np.abs(zmap_data[mask1_data])), mask2, fdr_corrected_full # return max absolute z-value in mask1 (means that no results above the threshold), empty mask2, and full FDR-corrected p-values
    else:
        # Get z-values within mask2
        zvals_in_mask2 = zmap_data[mask2]
        
        # Find the threshold as the smallest absolute z-value in mask2
        abs_zvals = np.abs(zvals_in_mask2)
        threshold = np.min(abs_zvals)
    
        print(f"FDR correction results:")
        print(f"  - Original voxels in mask1: {np.sum(mask1_data)}")
        print(f"  - Valid p-values: {np.sum(valid_pvals)}")
        print(f"  - Voxels surviving FDR correction: {np.sum(mask2)}")
        print(f"  - Threshold (min |z|): {threshold:.4f}")
        
        return threshold, mask2, fdr_corrected_full

# function to save the mask and threshold value
def save_mask_and_threshold(mask2: np.ndarray, 
                          reference_img_path: str,
                          output_mask_path: str,
                          threshold: float,
                          output_threshold_path: Optional[str] = None) -> None:
    """
    Save the FDR-corrected mask and optionally the threshold value.
    ...
    reference_img_path : str
        Path to reference image for header information
    output_mask_path : str
        Output path for the mask NIfTI file
    threshold : float
        Threshold value
    output_threshold_path : str, optional
        Output path for threshold text file
    """
    # Load reference image for header
    ref_img = nib.load(reference_img_path)
    
    # Create new NIfTI image with the mask
    mask_img = nib.Nifti1Image(mask2.astype(np.uint8), 
                              ref_img.affine, 
                              ref_img.header)
    
    # Save mask
    nib.save(mask_img, output_mask_path)
    print(f"Saved FDR-corrected mask to: {output_mask_path}")
    
    # Save threshold if path provided
    if output_threshold_path:
        with open(output_threshold_path, 'w') as f:
            f.write(f"FDR-corrected threshold: {threshold:.6f}\n")
        print(f"Saved threshold to: {output_threshold_path}")
...
def generate_nifti_report(
    ...
    zmap_path : str
        Path to the z_val map (NIFTI file).
    pmap_path : str
        Path to the p_val map (NIFTI file, values are positive).
    output_dir : str
    ...
    inclusive_mask (str): Path to the inclusive mask file, e.g., "tpl-MNI152NLin2009cAsym_brain_mask.nii.gz"
    ...
    # Get statistical thresholds from the z-map and p-map
    try:
        # Calculate threshold
        voxel_thresh, mask2, fdr_pvals = get_statistical_threshold(
            zmap_path, pmap_path, inclusive_mask, fdr_alpha=0.05
        )
            
        # Optionally save the new mask
        save_mask_and_threshold(
            mask2, 
            zmap_path,  # Use zmap as reference
    ...
    shutil.copy2(zmap_path, stat_img_path)
    create_output(
    ...
    with open(html_path, 'w') as f:
        ...
        <body>
            ...
            <div class="section">
                <h2>File Paths</h2>
                <table class="info-table">
                    <tr><td>Z_val Map:</td><td>{zmap_path}</td></tr>
                    <tr><td>p_val Map:</td><td>{zmap_path}</td></tr>
            ...
            <div class="section">
                <h2>Methods</h2>
                <p>Statistical analysis was conducted using the <a href="https://github.com/sundelinustc/IBMMA" target="_blank">IBMMA</a> package for neuroimaging data processing. Two-tailed statistical maps were thresholded at Z &gt; {voxel_thresh} 
                (corresponding to p<sub>FDR</sub> &lt; 0.05) with a minimum cluster extent threshold of {cluster_extent} contiguous voxels to control for multiple comparisons. All visualizations presented herein display only voxels that exceeded these statistical thresholds.</p>
...
def report_single(args):
    ...
    """
    # Arguments
    report_dir, result_dir, model_name, model_formula, filter_string, effect_interest, model_Subjects, labels_file, atlas_file, table1_site_var, table1_group_var, inclusive_mask = args
    ...
    try:
        ...
        stat_nifti = os.path.join(report_dir, "Meta", effect_interest, model_name, "nifti_results", "stouffers_z_score.nii.gz")
        ...
        if os.path.isfile(stat_csv):
            ...
            generate_nifti_report(
                subjects_csv_file=model_Subjects,
                zmap_path=  os.path.join(report_dir, "Meta", effect_interest, model_name, "nifti_results", "stouffers_z_score.nii.gz"),           # zmap path
                pmap_path=  os.path.join(report_dir, "Meta", effect_interest, model_name, "nifti_results", "stouffers_p_value.nii.gz"),  # pTFCE neg results
                output_dir= os.path.join(report_dir, "Meta", effect_interest, model_name),  # Output directory
                data_type=os.path.basename(report_dir), # get datatype though the folder name
                model_num=model_name,
                model_txt=model_formula,
                effect_interest=effect_interest,
                peak_region_min_percentage=10,
...
class HTML_Report_Meta:
    def __init__(self, num_processes=None):
        self.num_processes = num_processes if num_processes is not None else mp.cpu_count() # use all CPUs if not specified
          
    def report(self, report_dir, result_dir, model_name, model_formula, filter_string, model_Subjects, atlas, labels, table1_site_var, table1_group_var, inclusive_mask):
        """
        ...
        meta_path = os.path.join(report_dir, "Meta")
        list_effect_interest = [item for item in os.listdir(meta_path) 
                            if os.path.isdir(os.path.join(meta_path, item))]
            
        # Arguments
        my_args = [(report_dir, result_dir, model_name, model_formula, filter_string, effect_interest, model_Subjects, labels, atlas, table1_site_var, table1_group_var, inclusive_mask) for effect_interest in list_effect_interest]
